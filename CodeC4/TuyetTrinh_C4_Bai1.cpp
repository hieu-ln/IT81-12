#include <iostream>
using namespace std;
#define COUNT 10
//cau 1.1
struct Node
{
	int info;
	Node *left;
	Node *right;
};
Node *root;
//cau 1.2
void tree_empty()
{
	root = NULL;
}
//cau 1.3
void InterNode(Node *&p, int x)
{
	if (p == NULL)
	{
		p = new Node;
		p->info = x;
		p->left = NULL;
		p->right = NULL;
	}
	else
	{
		if (p->info == x)
			return;
		else if (p->info > x)
			return InterNode(p->left, x);
		else
			return InterNode(p->right, x);
	}
}
//cau 1.4
Node *search(Node *p, int x)
{
	if (p != NULL)
	{
		if (p->info == x)
			return p;
		else
			if (x > p->info)
				return search(p->right, x);
			else
				return search(p->left, x);
	}
	return NULL;
}
// ham searchStandFor
void searchStandFor(Node *&p, Node *&q)
{
	if (q->left == NULL)
	{
		p->info = q->info;
		p = q;
		q = q->right;
	}
	else
		searchStandFor(p, q->left);
}
//cau 1.5
int Delete(Node *&T, int x)
{
	if (T == NULL)
		return 0;
	if (T->info == x)
	{
		Node *p = T;
		if (T->left == NULL)
			T = T->right;
		else if (T->right == NULL)
			T = T->left;
		else
			searchStandFor(p, T->right);
		delete p;
		return 1;
	}
	if (T->info < x)
		return Delete(T->right, x);
	if (T->info > x)
		return Delete(T->left, x);
}
// cau 1.6
 void duyetNLR(Node *p)
 {
	 if (p != NULL)
	 {
		 cout << p->info << " ";
		 duyetNLR(p->left);
		 duyetNLR(p->right);
	 }
 }
//cau 1.7
 void duyetLNR(Node *p)
 {
	 if (p != NULL)
	 { 
		 duyetLNR(p->left);
		 cout << p->info << " ";
		 duyetLNR(p->right);
	 }
 }
 //cau.8
 void duyetLRN(Node *p)
 {
	 if (p != NULL)
	 {
		 
		 duyetLRN(p->left);
		 duyetLRN(p->right);
		 cout << p->info << " ";
	 }
 }
 // xuat
 void print2DUtil(Node *p, int space)
 {
	 if (p == NULL)
		 return;
	 space += COUNT;
	 print2DUtil(p->right, space);
	 cout << endl;
	 for (int i = COUNT; i < space; i++)
		 cout << " ";
	 cout << p->info << "\n";
	 print2DUtil(p->left, space);
 }
 // Wrapper over print2DUtil()
 void Process_Tree()
 {
	 print2DUtil(root, 0);
 }
 int main()
 {
	 int choice = 0;
	 int x, i;
	 Node *p;
	 system("cls");
	 cout << "---------MENU-------\n";
	 cout << "1. Khoi tao cay nhi phan tim kiem rong\n";
	 cout << "2. Them phan tu vao cay NPTK\n";
	 cout << "3. Tim phan tu co gia tir x trong cay NPTK\n";
	 cout << "4. Xoa phan tu co gia tri x trong cay NPTK\n";
	 cout << "5. Duyet cay NPTK theo LNR\n";
	 cout << "6. Duyet cay NPTK theo NLR\n";
	 cout << "7. Duyet cay NPTK theo LRN\n";
	 cout << "8. Xuat cay NPTK\n";
	 cout << "9. Thoat\n";
	 do{
		 cout << "Vui long chon so de thuc hien: \n";
		 cin >> choice;
		 switch(choice)
		 {
		 case 1:
			 tree_empty();
			 cout << "Ban vua khoi tao cay rong thanh cong\n";
			 break;
		 case 2:
			 cout << "Vui Long nhap gia tri x can them: ";
			 cin >> x;
			 InterNode(root, x);
			 cout << "Cay NPTK sau khi them la: ";
			 Process_Tree();
			 break;
		 case 3:
			 cout << "Vui Long nhap gia tri x can tim: ";
			 cin >> x;
			 p = search(root, x);
			 if (p != NULL)
				 cout << "Tim thay x = " << x << " trong cay NPTK\n";
			 else
				 cout << "Khong tim thay x = " << x << "trong cay NPTK\n";
			 break;
		 case 4:
			 cout << "Vui Long nhap gia tri x can xoa: ";
			 cin >> x;
			 i = Delete(root, x);
			 if (i == 0)
				 cout << "Khong tim thay x = " << x << " de xoa\n";
			 else
			 {
				 cout << "Da xoa thnah cong phan tu x = " << x << "trong cay NPTK\n";
				 cout << " NPTK sau khi xoa la: \n";
				 Process_Tree();
			}
			 break;
		 case 5:
			 cout << "Cay NPTK sau khi duyet LNR la: ";
			 duyetLNR(root);
			 cout << endl;
			 break;
		 case 6:
			 cout << "Cay NPTK sau khi duyet NLR la: ";
			 duyetNLR(root);
			 break;
		 case 7:
			 cout << "Cay NPTK sau khi duyet LRN la: ";
			 duyetLRN(root);
			 cout << endl;
			 break;
		 case 8:
			 cout << "Cay NPTK nhu sau: ";
			 Process_Tree();
			 cout << endl;
			 break;
		 case 9:
			 cout << "GOODBYE.......!\n";
			 break;
		 default:
			 break;
		 }
	 } while (choice != 9);
	 system("pause");
	 return 0;
 }